...
function! s:get_buffer_manager(bufname) abort "{{{
  ...
  if empty(l:bm)
    let l:bm = selector#modules#buffer#new_manager()
    let s:Buffer_cache[a:bufname] = l:bm
  endif
  return l:bm
endfunction "}}}

function! selector#buffer#open(config) abort "{{{
...
function! selector#buffer#close(bufname) abort "{{{
  let l:bm = s:get_buffer_manager(a:bufname)
  call l:bm.close()
endfunction "}}}

function! selector#buffer#wipeout(bufname) abort "{{{
  let l:bm = s:get_buffer_manager(a:bufname)
